...
{
    DefaultLayout = RDLC;
    RDLCLayout = 'Layout/TCSCPOSReceipt.rdl';

    dataset
    {
        dataitem(PaymentHeader; "Payment/Receipt.")
        {
            ...
            column(BankName_PaymentReceipt; PaymentHeader."Bank Name")
            {
            }
            column(ChequeNo_PaymentReceipt; PaymentHeader."Cheque No.")
            {
            }
            ...
            column(AccountType_PaymentReceipt; PaymentHeader."Account Type")
            {
            }
            column(AmountLCY_PaymentReceipt; ABS(PaymentHeader."Amount (LCY)"))
            {
            }
            dataitem(PaymentLine; "Payment/Receipt Bal. Line.")
            {
                ...
                column(TransactionDescription_PaymentReceiptBalLine; PaymentLine."Transaction Description")
                {
                }
            ...
            begin
                ...
                IF (PaymentHeader."Account Type" IN [PaymentHeader."Account Type"::"G/L Account",
                  PaymentHeader."Account Type"::"Bank Account"]) AND (PaymentHeader."Balance Account Type" IN
                   [PaymentHeader."Balance Account Type"::Customer, PaymentHeader."Balance Account Type"::Vendor])
                  THEN BEGIN
                    CASE PaymentHeader."Balance Account Type" OF
                        PaymentHeader."Balance Account Type"::Customer:
                            IF Customer.GET(PaymentHeader."Balance Account No.") THEN BEGIN
                        ...
                        PaymentHeader."Balance Account Type"::Vendor:
                            IF vendor.GET(PaymentHeader."Balance Account No.") THEN BEGIN
                                ...
                                COMPRESSARRAY(CustomerAddr);
                            END;
                        PaymentHeader."Balance Account Type"::"Bank Account":
                            IF bank.GET(PaymentHeader."Balance Account No.") THEN BEGIN
                                ...
                                COMPRESSARRAY(CustomerAddr);
                            END;
                        PaymentHeader."Balance Account Type"::"Fixed Asset":
                            IF fixed.GET(PaymentHeader."Balance Account No.") THEN BEGIN
                ...
                END
                ELSE BEGIN
                    CASE PaymentHeader."Account Type" OF
                        PaymentHeader."Account Type"::Customer:
                            IF Customer.GET(PaymentHeader."Account No.") THEN BEGIN
                                CustomerAddr[1] := COPYSTR(Customer.Name, 1, 30);
                                CustomerAddr[2] := Customer."Name 2";
                                CustomerAddr[3] := Customer.Address;
                                CustomerAddr[4] := Customer."Address 2";
                                ...
                                COMPRESSARRAY(CustomerAddr);
                            END;
                        PaymentHeader."Account Type"::"Staff Loan":
                            IF Customer.GET(PaymentHeader."Account No.") THEN BEGIN
                        ...
                        PaymentHeader."Account Type"::Vendor:
                            IF vendor.GET(PaymentHeader."Account No.") THEN BEGIN
                                CustomerAddr[1] := vendor.Name;
                                CustomerAddr[2] := vendor."Name 2";
                                CustomerAddr[3] := vendor.Address;
                                CustomerAddr[4] := vendor."Address 2";
                                ...
                                COMPRESSARRAY(CustomerAddr);
                            END;
                        PaymentHeader."Account Type"::"Bank Account":
                            IF bank.GET(PaymentHeader."Account No.") THEN BEGIN
                                CustomerAddr[1] := bank.Name;
                                CustomerAddr[2] := bank."Name 2";
                                CustomerAddr[3] := bank.Address;
                                CustomerAddr[4] := bank."Address 2";
                                ...
                                COMPRESSARRAY(CustomerAddr);
                            END;
                        PaymentHeader."Account Type"::"Fixed Asset":
                            IF fixed.GET(PaymentHeader."Account No.") THEN BEGIN
                                CustomerAddr[1] := fixed.Description;
                                CustomerAddr[2] := fixed."Description 2";
                                CustomerAddr[3] := fixed."Location Code";
                                CustomerAddr[4] := fixed."Responsible Employee";
                ...
                IF (PaymentHeader."Multiple Balance Account" = TRUE) OR (PaymentHeader."Multiple Account" = TRUE) THEN
                    detatype := TRUE
                ELSE
                    detatype := FALSE;
                IF PaymentHeader."Amount (LCY)" = 0 THEN
                    PaymentHeader."Amount (LCY)" := PaymentHeader.Amount;
                PaymentHeader.MODIFY;



                IF NOT CurrReport.PREVIEW AND reprint THEN BEGIN
                    //PaymentHeader."No. Printed" := PaymentHeader."No. Printed" + 1;
                    PaymentHeader.MODIFY;
                END;
            end;
            ...
            begin
                CompanyData.GET;
                //PaymentHeader.SETRANGE(PaymentHeader."No.", ReceiptNo);
            end;
        }
    }
    ...
    trigger OnPreReport()
    begin
        /*  ReceiptNo := PaymentHeader.GETRANGEMIN(PaymentHeader."No.");
         IF ReceiptNo = '' THEN
